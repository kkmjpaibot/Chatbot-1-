    ...
    async def process_message(
        ...
        try:
            ...
            # Get contact information
                ...
                if not state.name:
                    # Extract name from message (remove any numbers but preserve case)
                    import re
                    ...
                    # Preserve the original case of the name
                    state.name = name
                    return {
                        "type": "message",
                        "content": f"Thank you, {state.name}. What is your phone number?",
                        "next_step": "get_contact_info"
                    }
                else:
                    ...
                    if len(phone) < 9 or len(phone) > 12:
                        return {
                            "type": "message",
                            "content": "Please provide a valid phone number (9-12 digits).",
                            "next_step": "get_contact_info"
                        }
                    ...
                    # Log lead information (in a real app, this would be saved to a database)
                    logger.info(f"Lead generated: {state.name}, {state.phone}, Age: {state.age}, Coverage Level: {state.coverage_level}")
                    
                    # Format phone number for display
                    display_phone = f"{phone[:3]}-{phone[3:6]} {phone[6:8]} {phone[8:]}" if len(phone) > 8 else phone
                    
                    return {
                        "type": "message",
                        "content": (
                            f"Thank you, {state.name}! One of our agents will contact you shortly at {display_phone} "
                            "to discuss your medical coverage options and provide an exact quote.\n\n"
                            "Have a great day! 😊"
                        ),
                        "next_step": "end_conversation"
                    }
