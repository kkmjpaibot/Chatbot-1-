...
class PerlindunganComboCampaign:
    ...
    async def process_message(
        ...
        try:
            ...
            if isinstance(message, str):
                ...
                if message_lower == "main_menu":
                    ...
                    return self._create_response(
                        ...
                        message="You've been returned to the main menu. How can we assist you today?",
            ...
            # Handle get_package state
            if state.current_step == "get_package":
                # First, ensure we have a valid age
                if not hasattr(state, 'age') or not state.age or state.age < 18 or state.age > 60:
                    logger.warning("No valid age found, redirecting to age input")
                    state.current_step = 'get_age_manually'
                    return {
                        "type": "message",
                        "response": "I need to verify your age first. Please enter your age (18-60):",
                        "content": "I need to verify your age first. Please enter your age (18-60):",
                        "next_step": "get_age_manually"
                    }
                
                # Check if we have a valid numeric selection
                if not message_content.strip().isdigit():
                    logger.info("[DEBUG] No valid package selected yet, showing package buttons")
                    return self.create_button_response(
                ...
                if package_choice not in [1, 2, 3]:
                    return self.create_button_response(
                        message="❌ Invalid package selection. Please choose a valid package:",
                        button_type='package_selection',
                        campaign_data=state.user_data,
                        next_step='get_package'
                    )
                
                # Get age from user data
                age = state.user_data.get('age')
                if not age or not (18 <= int(age) <= 120):
                ...
                # Prepare response with premium details
                response_msg = (
                ...
                # Log and return the response with standardized confirmation buttons
                logger.info(f"[DEBUG] Sending package selection response: {response_msg}")
                return self.create_button_response(
                    message=response_msg,
            ...
            if state.current_step == "confirm_package":
                try:
                    if message_content.strip().lower() in ["yes", "y"]:
                        # User confirmed, ask if they want an agent to contact them
                        ...
                        return self.create_button_response(
                            ...
                            campaign_data=state.user_data,
                            next_step='follow_up_contact'
                        )# User didn't confirm, go back to package selection
                        state.current_step = "get_package"
                        return await self.process_message(user_id, "show packages", ws)
                except Exception as e:
            ...
            if state.current_step == "follow_up_contact":
                try:
                    if message_content.strip().lower() in ["yes", "y"]:
                        ...
                        return self.create_button_response(
                            message="No problem! If you have any questions later, feel free to ask. Is there anything else I can help you with?",
                            button_type='navigation',
                            campaign_data=state.user_data,
                            next_step='end_conversation'
                        )
